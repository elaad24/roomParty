...
def update_vote_count_in_vote_model_by_voteUserModel(sender, instance, created, **kwargs):
    ...
    votes_table_instance=VotesModel.objects.filter(room_key=triggered_instance.room_key)[0]
...
def cleanup_in_UserVotesModel_when_song_change(sender, instance,**kwargs):
    ...
    DB_instance=VotesModel.objects.filter(room_key=instance.room_key)

    if len(DB_instance)>=1  and DB_instance[0].active_song_id !=instance.active_song_id:
